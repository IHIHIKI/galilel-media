    ...
    if (feedEnabled(itemModel) && (feedData.feed || []).length) {
        ...
        try {
            ...
            if (itemModel.type === 'text' || itemModel.type === 'button') {
                ...
                insertTextApplyAutosize(_scene, itemModel, $item, $item.children[0]);
            } else if (itemModel.type === 'image') {
...
function getOptimizedImageUrl(itemModel, s) {
    ...
    if (itemModel.default_decorationGroup) {
        ...
        if (itemModel.default_imageSize == 'Cover') {
...
function insertTextDynamicWidthDynamicHeight(stageItem, itemModel, $outerDiv, $innerDiv, formattedTextInput, state) {
    var oldTextWidth = itemModel.width;
    var oldTextX = itemModel.left;
    var _w, _x, _h;

    if ($innerDiv.firstChild.tagName) {
        //don't use offsetWidth because it will give you rounded integer value
        oldTextWidth = pf($innerDiv.firstChild.getBoundingClientRect().width);
    }

    $innerDiv.style.whiteSpace = 'nowrap';
    $innerDiv.innerHTML = '<span style="vertical-align: middle;">' + formattedTextInput + '</span>';

    var newTextWidth = pf($innerDiv.firstChild.getBoundingClientRect().width);
    var newTextHeight = pf($innerDiv.firstChild.getBoundingClientRect().height);

    //setting outer and inner box new width
    _w = newTextWidth + itemModel.default_paddingLeft + itemModel.default_paddingRight;
    $outerDiv.style.width = _w + 'px';
    $innerDiv.style.width = (_w - pi(itemModel.default_paddingLeft) -
        pi(itemModel.default_paddingRight)) + 'px';

    if (getPropStateVal(itemModel, 'default_textAlign', state) === 'right') {
        _x = oldTextX - (newTextWidth - oldTextWidth);
        $outerDiv.style.left = /*stageItem.left +*/ _x + 'px';
    } else if (getPropStateVal(itemModel, 'default_textAlign', state) === 'center') {
        _x = oldTextX - (newTextWidth - oldTextWidth) / 2;
        $outerDiv.style.left = /*stageItem.left +*/ _x + 'px';
    } else {
        $outerDiv.style.left = oldTextX + 'px';
    }

    var newBoxHeight = newTextHeight + pi(itemModel.default_paddingTop) + pi(itemModel.default_paddingBottom);

    _h = newBoxHeight;
    $outerDiv.style.height = _h + 'px';
    $innerDiv.style.height = (_h - pi(itemModel.default_paddingTop) -
        pi(itemModel.default_paddingBottom)) + 'px';
}
...
function insertTextFixedWidthDynamicHeight(stageItem, itemModel, $outerDiv, $innerDiv, formattedTextInput, state) {
    ...
    var textFittedSpanHeight = pf($innerDiv.children[0].getBoundingClientRect().height);
    var newBoxHeight = textFittedSpanHeight + pi(itemModel.default_paddingTop) + pi(itemModel.default_paddingBottom);
...
function translateItem(item) {
    ...
    switch (itemModel.type) {
        case 'text':
        case 'button':
            insertTextApplyAutosize(stage, itemModel, $item, $item.children[0]);
            break;
        case 'image':
...
function getBannerInfo() {
    ...
    var tstyles = isBnPeak?{}:get_text_styles();
    return {
        thumbnail_frame: frames.thumbnail_frame,
        fallback_frames: frames.fallback_frames,
        w: glob_params.w,
        h: glob_params.h,
        mp4: glob_params.mp4,
...
function get_fit_style($i,i,st) {
    setItemState($i,i,$i.parentNode,st);
    var $tf = $i.children[0].children[0].style;
    return 'display:'+$tf.display+'; font-size:'+$tf.fontSize+'; height:'+$tf.height+';';
}

function get_text_styles() {
    var ret = [], nhd, $i;
    config.forEach(function (i) {
        if (i.type == 'text' || i.type == 'button') {
            $i = document.getElementById(i.id);
            nhd = { n:get_fit_style($i,i,'n'), h:i['h_on']?get_fit_style($i,i,'h'):'', d:i['d_on']?get_fit_style($i,i,'d'):'' };
            setItemState($i,i,$i.parentNode,'n');
            ret.push({id:i.id, default_textFit:nhd});
        }
    });
    return ret;
}
    ...
    config.forEach(function (i) {
        if (i.type == 'scene') {
            ...
            ret = {
                ...
                fallback_frames: i.fallback || i.mp4 ? fallback_frames : [],
            };
        }
    });
    return ret;
